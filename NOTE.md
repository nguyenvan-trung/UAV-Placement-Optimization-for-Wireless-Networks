...
- **Góc ngẩng (Elevation Angle):**
  $$\theta_{ij} = \arcsin\left(\frac{z_i}{d_{ij}}\right) \quad (\mathrm{rad})$$

- **Xác suất nhìn thẳng (Probability of Line-of-Sight - LoS):**
  $$P_{\mathrm{LoS}}(\theta_{ij}) = \frac{1}{1 + a \cdot \exp\left(-b(\theta_{ij} - a)\right)}$$

  _(với $a, b$ là các hằng số môi trường đô thị - Urban Dense: $a = 9.61, b = 0.16$)_.

- **Suy hao đường truyền (Pathloss):**
  $$PL_{\mathrm{LoS}}(d_{ij}) = 20 \log_{10}\left(\frac{4\pi f d_{ij}}{c}\right) + \eta_{\mathrm{LoS}}$$
  $$PL_{\mathrm{NLoS}}(d_{ij}) = 20 \log_{10}\left(\frac{4\pi f d_{ij}}{c}\right) + \eta_{\mathrm{NLoS}}$$
  $$PL_{ij} = P_{\mathrm{LoS}}(\theta_{ij}) \cdot PL_{\mathrm{LoS}}(d_{ij}) + \left(1 - P_{\mathrm{LoS}}(\theta_{ij})\right) \cdot PL_{\mathrm{NLoS}}(d_{ij})$$

- **Tỷ số Tín hiệu trên Nhiễu (SNR):**
  $$SNR_{ij} = P_{\mathrm{tx}} - PL_{ij} - N_0$$
  *(Trong đó $P_{\mathrm{tx}}$ là công suất phát và $N_0$ là công suất tạp âm nền.)*

### 2.2. Chi Tiết Các Hàm Mục Tiêu (Fitness Objectives)

Hàm Fitness tổng hợp (Weighted Sum Approach):
$$\mathrm{Fitness} = w_1 \cdot f_1 - w_2 \cdot f_2 - w_3 \cdot f_3 \quad (w_1 + w_2 + w_3 = 1)$$

1. **Hàm Phủ Sóng ($f_1$ - Maximize Coverage Rate):**
   $$f_1 = \frac{1}{M} \sum_{j=1}^{M} \mathbb{I}\left( \max_{i=1 \dots N} (SNR_{ij}) \ge SNR_{\mathrm{th}} \right)$$

   *(Người dùng $j$ được phủ sóng nếu có ít nhất một UAV cung cấp $SNR \ge SNR_{\mathrm{th}}$.)*

2. **Hàm Năng Lượng ($f_2$ - Minimize Energy Consumption):**
   $$f_2 = \sum_{i=1}^{N} \left( P_{\mathrm{hover}}(z_i) + P_{\mathrm{transmit},i} \right)$$

   *(Trong đó công suất nâng $P_{\mathrm{hover}}(z_i)$ tăng dần theo độ cao $z_i$.)*

3. **Hàm Nhiễu Chồng Lấp ($f_3$ - Minimize Interference):**
   $$f_3 = \sum_{i=1}^{N} \sum_{k=i+1}^{N} \mathrm{AreaOverlap}(U_i, U_k)$$

   *(Diện tích giao nhau của vùng phủ mặt đất giữa UAV $i$ và UAV $k$.)*
...
$$X^{\mathrm{op}} = L + U - X$$
[cite: 1]
So sánh giá trị Fitness giữa $X$ và $X^{\mathrm{op}}$, giữ lại $N$ cá thể tốt nhất giúp tăng tốc độ hội tụ ban đầu[cite: 1].
...
$$X_i(t+1) = X_i(t) + \alpha \oplus \mathrm{Levy}(\beta)$$
[cite: 1]
$$\mathrm{Levy}(s) \sim |s|^{-1-\beta}, \quad 0 < \beta \le 2$$
[cite: 1]
...
| :------------------------------------- | :------------------------------------------ | :------------------------------------------------- |
| **Kích thước không gian**              | $1000\,\mathrm{m} \times 1000\,\mathrm{m}$[cite: 1] | $1000\,\mathrm{m} \times 1000\,\mathrm{m}$[cite: 1]        |
...
| **Độ cao UAV ($z_{\min} - z_{\max}$)** | $50\,\mathrm{m} - 200\,\mathrm{m}$[cite: 1]         | $50\,\mathrm{m} - 200\,\mathrm{m}$[cite: 1]                |
| **Tần số sóng 5G**                     | $3.5\,\mathrm{GHz}$ (Sub-6)[cite: 1]           | $3.5\,\mathrm{GHz}$ (Sub-6)[cite: 1]                  |
| **Ngưỡng kết nối $SNR_{\mathrm{th}}$**   | $5\,\mathrm{dB}$                               | $5\,\mathrm{dB}$                                      |
...
- **Opposition-Based Learning - OBL (Cải tiến 1):**
  - _Lý do:_ Khắc phục điểm yếu của khởi tạo ngẫu nhiên. OBL sinh các nghiệm đối lập song song ($X^{\mathrm{op}} = a + b - X$)[cite: 1], giúp thuật toán "quét" đối ứng toàn bộ không gian 3D, tránh việc tất cả cá thể bị dồn về một góc bản đồ ở thế hệ ban đầu[cite: 1].
